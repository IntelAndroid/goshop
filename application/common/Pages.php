...
class Pages
{
    ...
    public $uri;//动态url
    public $pageNum; //总页数
    public $listNum = 6;//显示页码按钮数量
    public $render;//分页后的html模板
    public $data;//分页后渲染到模板的数据

    /* * 初始化分页数据 *$sdata 待分页的数据 * $listRows 每页记录数 */
    public function __construct($sdata, $listRows = 6)
    {
        $this->total = count($sdata);
        $this->listRows = $listRows;
        $this->uri = $this->getUri();
        $this->page = !empty($_GET["page"]) ? (is_numeric($_GET['page']))?$_GET['page']:1 : 1;
    ...
    {
        $html = "";
        if ($this->page == 1) $html .= " <a style='margin: 10px;' class='current btn disabled'>首 页</a>"; else $html .= " <a class='btn btn-primary-outline' href='{$this->uri}&page=1'>首 页</a>";
        return $html;
    }
    ...
    {
        $html = "";
        if ($this->page == 1) $html .= " <a class='current btn disabled'>上一页</a>"; else $html .= " <a class='btn btn-primary-outline' href='{$this->uri}&page=" . ($this->page - 1) . "'>上一页</a>";
        return $html;
    }
    ...
    {
        $html = "";
        if ($this->page == $this->pageNum) $html .= " <a class='current btn disabled'>下一页</a>"; else $html .= " <a class='btn btn-primary-outline' href='{$this->uri}&page=" . ($this->page + 1) . "'>下一页</a>";
        return $html;
    }
    ...
    {
        $html = "";
        if ($this->page == $this->pageNum) $html .= " <a class='current btn disabled'>尾 页</a>"; else $html .= " <a class='btn btn-primary-outline' href='{$this->uri}&page=" . ($this->pageNum) . "'>尾 页</a>";
        return $html;
    }
    ...
    function pageHtml()
    {
        $html = "<div class='cl mt-20 text-c' style='position: fixed;bottom: 2%;margin: 0 auto;width: 100%;text-align: center'>";
        $html .= "<span >共有<b>{$this->total}</b>条记录</span>";
